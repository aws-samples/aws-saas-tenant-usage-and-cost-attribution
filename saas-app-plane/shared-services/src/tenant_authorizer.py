...
def lambda_handler(event, context):
    ...
    service_identifier = event['headers'].get('x-service-identifier')
    logger.info('service_identifier: ' + service_identifier)
    ...
    # tenant admin is only allowed to get and create users
    if (auth_manager.isTenantAdmin(user_role)):
        policy.allowAllMethods()
    else:
        policy.denyMethod(HttpVerb.GET, "users/*")
        policy.denyMethod(HttpVerb.POST, "users/*")

    authResponse = policy.build()

    if (service_identifier != 'ProductReviewService'):
            
        #   Generate STS credentials to be used for FGAC
        #   Important Note: We are generating STS token inside Authorizer to take advantage of the caching behavior of authorizer
        iam_policy = auth_manager.getPolicyForUser(service_identifier, tenant_id, region, aws_account_id)
        logger.info(iam_policy)

        role_arn = authorizer_access_role
        # "arn:aws:iam::{}:role/authorizer-access-role".format(aws_account_id)

        # Prepare the assume_role arguments
        assume_role_kwargs = {
            "RoleArn": role_arn,
            "RoleSessionName": "tenant-aware-session"
        }

        # Include the policy if it is defined
        if iam_policy:
            assume_role_kwargs["Policy"] = iam_policy

        # Assume the role with or without the policy
